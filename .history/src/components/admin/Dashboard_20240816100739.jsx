...
function Dashboard({ setOutlet }) {
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const { customerList, orderList, productList } = useSelector(
    (state) => state.admin
  );
  ...
  return (
    <div className="bg-background text-primary-foreground min-h-screen p-8">
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
        <div
          onClick={() => setOutlet("Customer's List")}
          className="bg-secondary text-secondary-foreground p-6 rounded-lg shadow-lg transition-transform transform hover:scale-105"
        >
        ...
        <div
          onClick={() => setOutlet("Product List")}
          className="bg-accent text-accent-foreground p-6 rounded-lg shadow-lg transition-transform transform hover:scale-105"
        >
        ...
        <div
          onClick={() => setOutlet("COrder List ")}
          className="bg-destructive text-destructive-foreground p-6 rounded-lg shadow-lg transition-transform transform hover:scale-105"
        >
