...
import { updateProductItem } from "../../store/actions/adminActions";
...
export default function ProductList() {
  ...
  function handleProcessRowUpdateError(){
    toaster.danger("error updating")
  }
 const processRowUpdate = React.useCallback(
   (newRow) => {
     console.log(newRow);
     // toaster.success("updated successfully");
     dispatch(updateProductItem(newRow));
   },
   [dispatch]
 );
  ...
  const columns = [
      ...
      renderCell: (params) => {
        return (
          <div style={{ display: "flex", alignItems: "center", gap: "5px" }}>
            <span>
              <img src={params.row.img[0]} width={30} height={30} alt="" />
            </span>
    ...
    },
    {
      ...
      renderCell: (params) => {
        return (
          <div
            onClick={() => processRowUpdate(processRowUpdate)}
            className={`${styles.quantityCell} ${styles.available }`}
          >
            {Up}
          </div>
        );
  ...
  return (
    ...
    {opens && <ProductDetails back={setOpens} productDetails={selectedRow}/>}
    </>
  );
}
